...
function autocomplete(inp, game) {
    ...
    inp.addEventListener("input", function (e) {
        ...
        for (i = 0; i < players.length; i++) {
            ...
            if (e.target.value.toUpperCase() == players[i].name.slice(0, e.target.value.length).toUpperCase()) {
                ...
                b.innerHTML += `<div class='self-center'>
                                    <span class='font-bold'> ${players[i].name.slice(0, e.target.value.length)}</span><span class>${players[i].name.slice(e.target.value.length)}</span>
